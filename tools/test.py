...
class Evaluator:
    ...
    def test(self, output_dir):
        ...
        prog_bar.close()
        sec_per_example = (time.time() - start_time) / len(self.eval_dataloader.dataset)

        self._log_string('sec_per_example: %.4f second.' % sec_per_example)
    ...
    def evaluate(self):
        if self.eval_all:
            for checkpoint_path in self.checkpoint_list:
                ...
                tmp_output_dir = os.path.join(train_dir, file_name, 'results')
                # self.output_dir = train_dir
                # tmp_output_dir = os.path.join(self.output_dir, file_name)
                self.test(tmp_output_dir)
        else:
