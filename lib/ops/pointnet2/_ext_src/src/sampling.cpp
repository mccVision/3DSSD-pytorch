...
at::Tensor furthest_point_sampling_with_dist(at::Tensor features, const int nsamples)
{
  CHECK_CONTIGUOUS(features);
  CHECK_IS_FLOAT(features);

  at::Tensor output =
      torch::zeros({features.size(0), nsamples},
                   at::device(features.device()).dtype(at::ScalarType::Int));

  at::Tensor tmp =
      torch::full({features.size(0), features.size(1)}, 1e10,
  ...
  if (features.type().is_cuda())
  {
    furthest_point_sampling_with_dist_kernel_wrapper(
        features.size(0), features.size(1), nsamples, features.data<float>(),
        tmp.data<float>(), output.data<int>());
  }
  else{
    AT_CHECK(false, "CPU not supported");
