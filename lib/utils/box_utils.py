...
def batch_boxes_to_corners_3d(boxes3d):
    """
    batch boxes_to_corners3d
    Args:
        boxes3d: ([bs, N, 7])
    Returns:
        corners3d: ([bs, N, 8, 3])
    """
    bs, points_num = boxes3d.shape[0], boxes3d.shape[1]
    corners3d = boxes3d.new_zeros((bs, points_num, 8, 3))
    for i in range(bs):
        cur_boxes3d = boxes3d[i]
        cur_corners = boxes_to_corners_3d(cur_boxes3d)
        corners3d[i] = cur_corners
...
def mask_boxes_outside_range_numpy(boxes, limit_range, min_num_corners=1):
    ...
    # centers = boxes[:, :3]
    # mask = ((centers >= limit_range[:3]) & (centers <= limit_range[3:6])).all(axis=1)  # (N)
    # mask = mask > 0
